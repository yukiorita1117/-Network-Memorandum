...
- 第 1 層 物理層
  ...
  つまり、実際に銅線、光ファイバー、無線 LAN などの回線を通し、アナログ信号 or デジタル信号を使用し、通信を行う。<br/>
  もちろんこの際に障害が起こる。ケーブルを通信の際、使用しているためである。このケーブルの抵抗が起因する。
  ...
  - 衝突は「ケーブル上を２つのデータが同時に流れた場合発生する。」
    → 衝突が発生する可能性がある範囲のことを、**衝突ドメイン**といい、これを回避するために第二層のネットワーキングデバイスが存在する。

  - ネットワーキングメディアには、**同軸**、**UTP**、**光ファイバの３種類**。
    → 実際に LAN で多く使われているのは、UTP と光ファイバ。
  ...
  - ネットワーキングデバイス
    ...
    - リピータ
      ...
      補足：リピータ・ハブは制御を全く行わないので、送られてくる信号をすべてに流す共有メディア環境を作り出してしまう。

  - 機器とメディアの配置を物理トポロジという,が今回はあまりいらんかな。
...
- 複数のパソコンからのデータが LAN に流れることになる。通信を制御する機器が必要。
  →「ネットワーキングデバイス」

- ネットワーキングデバイスは第 1~3 層にそれぞれ複数存在する。

- ネットワークモデルがそれぞれの処理を階層ごとに分けているのは、考えやすくなるように、役割を分けているから。
  → なので階層ごとにネットワーキングデバイスも役割を抑えた方が良い。
